...
export default function AuthCallback() {
  ...
  useEffect(() => {
    ...
    if (!isElectron) {
      ...
      const callbackUrl = `huckepack://auth-callback?${new URL(currentUrl).searchParams.toString()}`;

      try {
        window.location.href = callbackUrl;
        return;
      } catch (error) {
        console.error("Failed to redirect to Electron:", error);
      }
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const checkAuth = () => {
      ...
      const hasCallbackParams = urlParams.has("code") || urlParams.has("session_token") || urlParams.has("redirect");
