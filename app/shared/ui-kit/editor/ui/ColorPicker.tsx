...
import { ColorPicker as RBCColorPicker, useColorState } from "react-beautiful-color";
...
export interface ColorPickerProps {
  ...
  onModeToggle: () => void;
  onTailwindColorChange: (color: string, shade: number) => void;
}
...
export function ColorPicker({
  ...
  isArbitraryMode,
  onModeToggle,
  onTailwindColorChange,
}: ColorPickerProps) {
  const hexValue = value?.startsWith("#") ? value : value ? `#${value}` : "#3b82f6";
  const [{ colorState }, setColor] = useColorState({
    type: "hex",
    value: hexValue,
  });

  useEffect(() => {
    if (colorState.hex !== hexValue) {
      setColor({ type: "hex", value: hexValue });
    }
  }, [hexValue, colorState.hex]);

  const handleColorChange = (color: { getHex: () => string }) => {
  ...
  const selectedShade = useMemo(() => {
    if (!isArbitraryMode && value) {
      for (const color of TAILWIND_COLORS) {
        for (const shade of color.shades) {
          const hex = COLOR_MAP[color.name]?.[shade];
          if (hex === value) {
            return { color: color.name, shade };
          }
        }
      }
    }
    return null;
  }, [value, isArbitraryMode]);
  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between">
        ...
        <IconToggle
          icon={isArbitraryMode ? IconPalette : IconColorSwatch}
          isActive={isArbitraryMode}
          onChange={onModeToggle}
        />
      </div>

      {isArbitraryMode ? (
        <RBCColorPicker color={{ type: "hex", value: colorState.hex }} onChange={handleColorChange}>
          <RBCColorPicker.Saturation className="mb-3 flex-1 rounded-md" />
          <div className="flex items-center gap-3">
            <RBCColorPicker.EyeDropper />
            <div className="flex flex-1 flex-col gap-3">
              <RBCColorPicker.Hue className="h-4" />
              {showOpacity && <RBCColorPicker.Alpha className="h-4" />}
            </div>
          </div>
        </RBCColorPicker>
      ) : (
            ...
            {TAILWIND_COLORS.map((color) => (
              <div key={color.name} className="flex flex-col gap-1">
                {color.shades.map((shade) => {
                  const hex = COLOR_MAP[color.name]?.[shade];
                  const isSelected = selectedShade?.color === color.name && selectedShade?.shade === shade;
                  return (
                    <button
                      key={shade}
                      type="button"
                      className={`h-4 w-full rounded border transition-all hover:scale-110 ${
                        isSelected ? "ring-2 ring-primary-500 ring-offset-1" : ""
                      }`}
                      style={{ backgroundColor: hex }}
                      onClick={() => onTailwindColorChange(color.name, shade)}
